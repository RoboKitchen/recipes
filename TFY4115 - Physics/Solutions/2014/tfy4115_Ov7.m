...
% Verlet-integrasjon av tyngdependel med dempning.
...
omega0 = 1.0;   
gamma = 0.0001;   % Velger veldig svak demping
...
aa = (2-omega0^2*dt^2)/(1+gamma*dt);
bb = (1-gamma*dt)/(1+gamma*dt);
cc = 2/(1+gamma*dt);
dd = omega0^2*dt^2/(1+gamma*dt);
...
for n=2:(length(t)-1)
   theta(n+1)=cc*theta(n)-bb*theta(n-1)-dd*sin(theta(n)); % Ikke-lineære
   ...
   thetalin(n+1)=aa*thetalin(n)-bb*thetalin(n-1);  % Lineære
      % Sjekker første og andre nullgjennomgang pos -> neg:
